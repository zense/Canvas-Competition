...
canvas.setup = function() {
	...
	// Set the default fill mode to Stroke
	this.setMode();
};

// @author Shubhayu Das
// Set the mode to Fill/Stroke
canvas.setMode = function(mode = "stroke") {
	if (mode === "fill") {
		this.ctx.fillMode = this.ctx.fill;
	} else {
		this.ctx.fillMode = this.ctx.stroke;
	}
};

// @author Shubhayu Das
// Change the color of the shape(s) drawn unless set to some other color
